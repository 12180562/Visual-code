...
class Visualization:
    ...
    def ploting(self, name = None):
        ...
        axes[1,0].axis([0, self.time, 0, max(dcpa1_y)]) # 로그 스케일 범위 설정
        ...
        ax2.axis([0, self.time, min(tcpa1_y), max(tcpa1_y)])
...
frm_info = pd.read_csv('./csv/ais.csv')
