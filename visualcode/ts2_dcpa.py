...
class Visualization:
    ...
    C1x, C1y, C2x, C2y, C3x, C3y, C4x, C4y, C5x, C5y, C6x, C6y, DCPA1, DCPA2, TCPA1, TCPA2, cri1, cri2, Heading, time):
    ...
    def ploting(self, name = None):
        ...
        axes[0,0].set_yticks(np.arange(-100, 500, 50))
        axes[0,0].set_aspect('equal')
        axes[0,0].grid(linestyle='-.')
        axes[0,0].set_xlabel('X axis (m)', labelpad=1, fontsize=15)
        axes[0,0].set_ylabel('Y axis (m)', labelpad=1, fontsize=15)
        axes[0,0].axis([-300, 300, -100, 500])
        ...
        axes[1,0].axis([0, self.time, 0, max(max(dcpa1_y), max(dcpa2_y))]) # 로그 스케일 범위 설정
        ...
        ax2.axis([0, self.time, min(min(tcpa1_y), min(tcpa2_y)), max(max(tcpa1_y), max(tcpa2_y))])
...
frm_info = pd.read_csv('./csv_h,s_p/frm.csv')
wp_info = pd.read_csv('./csv_h,s_p/wp.csv')
cri_info = pd.read_csv('./csv_h,s_p/cri.csv')
vo_info = pd.read_csv('./csv_h,s_p/vo.csv')
ais_info = pd.read_csv('./csv_h,s_p/ais.csv')
...
for i in range(data_len):
    ...
    dcpa1_list, dcpa2_list, tcpa1_list, tcpa2_list, cri1_list, cri2_list, Heading_list, time)
